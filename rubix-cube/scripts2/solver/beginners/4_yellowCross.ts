...
export function solveYellowCross(cube: RubixCube): void {
...
function solveSingleYellowDot(cube: RubixCube): void {
   const yellowEdges = getEdges(cube.top).filter((tile) => tile.color === TileColor.yellow);
...
function solveYellowL(cube: RubixCube): void {
   ...
   const areYellowEdgesInLine = yellowEdges[0].row === yellowEdges[1].row || yellowEdges[0].col === yellowEdges[1].col;
