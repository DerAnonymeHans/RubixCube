...
function isTopRightCornerSolved(topRightCorner: CornerTile[], face: CubeFace) {
   return (
      topRightCorner.find((tile) => tile.color === TileColor.white && tile.face.facePosition === FacePosition.top) !== undefined &&
      topRightCorner.every(
         (tile) =>
            tile.color === TileColor.white || tile.color === face.getCell(1, 1) || tile.color === face.neighbours.right.face.getCell(1, 1)
      )
   );
}
