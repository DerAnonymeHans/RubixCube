...
import { CornerTile, getCornerTiles, getEdgeTiles, getEdges, mapLocalToGlobalRotation } from "../helper";
import { LocalRelation } from "../types";
import { DEFAULT_ROTATION } from "./0_solver";
...
export function solveFirstTwoLayers(cube: RubixCube) {
   ...
   while (numberOfSolvedEdges < 4) {
      ...
      for (let j = 0; j < 4; j++) {
         ...
         cube.rotate(RotationCommand.fromFacePosition(FacePosition.top, DEFAULT_ROTATION, "Rotate top"));
      }
...
function liftWrongEdge(cube: RubixCube) {
   for (const neighbour of cube.top.neighbourList) {
      ...
      cube.rotate(RotationCommand.fromFacePosition(right.facePosition, RotationDirection.right, "Lift wrong edge"));
      ...
      const topRotationDirection = doesTopLeftCornerMatchEdge ? RotationDirection.right : RotationDirection.left;
...
function checkIfEdgeIsOnValidPosition(cube: RubixCube, color1: TileColor, color2: TileColor): boolean {
   for (const neighbour of cube.top.neighbourList) {
      const front = neighbour.relation.face;
      const right = front.neighbours.right.face;
...
function liftWrongCorner(cube: RubixCube): boolean {
   for (const neighbour of cube.top.neighbourList) {
      ...
      cube.rotate(RotationCommand.fromFacePosition(right.facePosition, RotationDirection.right, "Lift wrong corner"));

      const isTopLeftCornerWhite =
         getCornerTiles(front, front.neighbours.left.face, cube.top).find((x) => x.color === TileColor.white) !==
         undefined;

      const bottomRightCornerColors = getNonWhiteCornerColors(bottomRightCorner);
      const isMatchingEdgeAtFront = getEdgeTiles(front, cube.top).every((x) => bottomRightCornerColors.includes(x));
      const isMatchingEdgeAtBack = getEdgeTiles(front.oppositeFace, cube.top).every((x) =>
         bottomRightCornerColors.includes(x)
      );

      let topRotationDirection = isMatchingEdgeAtFront ? RotationDirection.right : RotationDirection.left;
      if (!isMatchingEdgeAtBack && !isMatchingEdgeAtFront) {
         topRotationDirection = isTopLeftCornerWhite ? RotationDirection.right : RotationDirection.left;
      }
...
function solveCurrentFirstTwoLayers(cube: RubixCube): number {
   ...
   for (const neighbour of cube.top.neighbourList) {
      const front = neighbour.relation.face;
      const right = front.neighbours.right.face;
      const [top_frontTile, top_rightTile, topTile] = getCornerTiles(front, front.neighbours.right.face, cube.top);
      ...
      // corner in bottom, edge in middle
      {
         ...
         if (bottomInPlace && front.getCell(1, 2) === right.faceColor && right.getCell(1, 0) === front.faceColor) {
