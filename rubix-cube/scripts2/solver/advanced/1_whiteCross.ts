...
function bringTopFaceWhiteEdgesToBottomFace(cube: RubixCube, allowFaceRotations = false) {
   ...
   for (const neighbour of cube.top.neighbourList) {
      ...
      const isWhiteEdgeOnTopFace = face.neighbours.top.getTouchingArrayUnaligned()[1] === TileColor.white;
...
function bringSideFaceWhiteEdgesToBottomFace(cube: RubixCube, allowSuboptimal: boolean): number {
   ...
   for (const neighbour of cube.bottom.neighbourList) {
      ...
      const edgeColor = face.getNeighbour(whiteEdgePosition).relation.getTouchingArrayUnaligned()[1];
...
function solveWhiteEdgeOnCorrectFace(cube: RubixCube, face: CubeFace, whiteEdgePosition: LocalRelation) {
   if (whiteEdgePosition === LocalRelation.left) {
      ...
      cube.rotateMultipleTimes(getRotationForEdgeToDifferentEdgeOnFace(face, whiteEdgePosition, LocalRelation.right));
...
function solveWhiteEdgeOnWrongSideFace(
   ...
   if (face.neighbours.left.face.getCell(1, 1) === edgeColor) {
      const rotationsToLeftEdge = getRotationForEdgeToDifferentEdgeOnFace(face, whiteEdgePosition, LocalRelation.left);
      ...
      cube.rotate(RotationCommand.fromFacePosition(face.neighbours.left.face.facePosition, RotationDirection.right));

      if (faceAlreadySolved) cube.rotateMultipleTimes(rotationsToLeftEdge.map((x) => invertRotation(x)));
   } else if (face.neighbours.right.face.getCell(1, 1) === edgeColor) {
      ...
      // after this the white edge is on the bottom face
      cube.rotate(RotationCommand.fromFacePosition(face.neighbours.right.face.facePosition, RotationDirection.left));

      if (faceAlreadySolved) cube.rotateMultipleTimes(rotationsToRightEdge.map((x) => invertRotation(x)));
   } else {
      ...
      if (whiteEdgePosition === LocalRelation.left) {
         ...
         const rotations = getRotationForEdgeToDifferentEdgeOnFace(face, whiteEdgePosition, LocalRelation.right);
...
function bringBottomFaceWhiteEdgesToTheirCenters(cube: RubixCube) {
